 ...
 * Uses Helius RPC getProgramAccounts (FREE with Helius API key)
 */
async function fetchHolderCounts(mints: string[]): Promise<Map<string, number>> {
  ...
  if (!apiKey || mints.length === 0) {
    console.log("[API] No HELIUS_API_KEY - skipping holder counts")
    return holderMap
  }
  ...
  console.log(`[API] Fetching holder counts for ${mintsToFetch.length} tokens via Helius RPC`)

  // Process in small batches (getProgramAccounts can be resource-intensive)
  const BATCH_SIZE = 5
  let fetchedCount = 0
  ...
  for (let i = 0; i < mintsToFetch.length; i += BATCH_SIZE) {
    ...
    const batchPromises = batch.map(async (mint) => {
      const holders = await fetchHolderCount(mint)
  ...
  console.log(`[API] Holder counts fetched: ${fetchedCount}/${mintsToFetch.length}`)
